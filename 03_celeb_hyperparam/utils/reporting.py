...
def get_class_distrutoins(ds):
    # Train Class Distribution
    n_classes = ds.Y.shape[1]
    train_class_sum = np.sum(ds.Y[ds.train_idxs], axis=0)
    df_train_class_sum = pd.DataFrame([train_class_sum], index=['train_distribution'], columns=np.arange(n_classes).astype(str))

    # Test Class Distribution
    test_class_sum = np.sum(ds.Y[ds.test_idxs], axis=0)
    df_test_class_sum = pd.DataFrame([test_class_sum], index=['test_distribution'], columns=np.arange(n_classes).astype(str))
    return df_train_class_sum, df_test_class_sum
