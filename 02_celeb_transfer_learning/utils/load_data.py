...
def get_dataframe(data_dir, face_cascade, eye_cascade, num_entities=4, num_images=25, rnd_seed=1, img_dims=100):
    ...
    for name in celebs:
        samples = os.listdir(os.path.join(data_dir, name))
...
def get_images(df, entity, data_dir, face_cascade, eye_cascade, retrain_path=None, corpus_dir=None, bottle_dir=None, img_dims=100, normalized=None, mean_image=None):
    ...
    # face_images, face_images_list = align_images(entity, images_list, data_dir, corpus_dir, img_dims)
    # face_images, face_images_list = align_multi_images(entity, images_list, data_dir, corpus_dir, img_dims)


    if normalized == True:
        print("get_images.normalize==TRUE")
    ...
    if corpus_dir != None:
        for idx, image in enumerate(face_images):
            ...
            #io.imsave(os.path.join(destin_dir, face_images_list[idx]), image.astype('uint8'))
